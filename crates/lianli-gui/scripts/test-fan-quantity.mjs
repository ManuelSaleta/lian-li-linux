...
const device = { device_id: "hid:6243168001:port2", serial: "6243168001", max_fan_quantity: 4 };
assert.equal(stageFanQuantity(config, device, 3), 3);
const request = JSON.parse(JSON.stringify({ method: "SetConfig", params: { config } }));
assert.deepEqual(request.params.config.ene6k77, { "6243168001": { fan_quantities: { "2": 3 } } });
...
assert.deepEqual(Object.keys(config.ene6k77[device.serial].fan_quantities), ["2"]);
